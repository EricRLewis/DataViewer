...
public class PR1View  extends JComponent implements ActionListener, ChangeListener, GLEventListener, MouseListener, MouseMotionListener, TableModelListener {
	...
	private JComboBox<String> xCombo = null;
	private JComboBox<String> yCombo = null;
	...
	private int selectedXCol = 0;
	private int selectedYCol = 1;
	...
	public PR1View(PR1Model m) {
		...
		xCombo = new JComboBox<String>(columnNames);
		xCombo.addActionListener(this);
		yCombo = new JComboBox<String>(columnNames);
		yCombo.addActionListener(this);
		comboPanel = new JPanel();
		comboPanel.setLayout(new BorderLayout());
		comboPanel.add(xCombo, BorderLayout.WEST);
		comboPanel.add(yCombo, BorderLayout.EAST);
	...
	public void display(GLAutoDrawable drawable) {
		...
		if ((data = model.getTwoColumnData(selectedXCol, selectedYCol)) != null)  {
			...
			gl.glUniform1f(location, model.getScaleX(selectedXCol));
			...
			gl.glUniform1f(location, model.getScaleY(selectedYCol));
			...
			gl.glUniform1f(location, model.getTranslationX(selectedXCol));
			...
			gl.glUniform1f(location, model.getTranslationY(selectedYCol));
		...
		if ((data = model.getSelectedTwoColumnData(selectedXCol, selectedYCol)) != null) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "comboBoxChanged":
			selectedXCol = xCombo.getSelectedIndex();
			selectedYCol = yCombo.getSelectedIndex();
			repaint();
			break;
		case "BB":
			currentColor = 0;
	 ...
	 * @param update If true, update the selection based on the rubberband rectangle.
	 */
	private void setRubberBandData(int x1, int y1, int x2, int y2, boolean update) {
		...
		xmin = (xmin - model.getTranslationX(selectedXCol)) / model.getScaleX(selectedXCol);
		xmax = (xmax - model.getTranslationX(selectedXCol)) / model.getScaleX(selectedXCol);
		ymin = (ymin - model.getTranslationY(selectedYCol)) / model.getScaleY(selectedYCol);
		ymax = (ymax - model.getTranslationY(selectedYCol)) / model.getScaleY(selectedYCol);
		...
		if (update) {
			model.addSelectedRows(xmin, xmax, ymin, ymax, selectedXCol, selectedYCol);
		}
	}
	
	/**
	 * Updates the comboBox options when a new table is loaded. 
	 */
	public void updateComboBox(){
		...
		for (int i = 0; i < numCols; i++){
			newCols[i] = model.getColumnName(i);
		}
		xCombo.setModel(new DefaultComboBoxModel<String>(newCols));
		yCombo.setModel(new DefaultComboBoxModel<String>(newCols));
		
		if(yCombo.getModel().getSize() > 1){
